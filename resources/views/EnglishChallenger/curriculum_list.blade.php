...
<head>
    ...
    <style>
        ...
        .input-group.rounded input[type="text"]:focus {
            outline: none;
        ...
        :-ms-input-placeholder {
            color: #07294D;
            /* Change placeholder color to gray for Microsoft Edge */
        }
...
<body @if (isset($GTest)) class="sidebar-collapse" @endif>
    @if (!isset($GTest))
    <header class="d-flex justify-content-end pl-4" id="header">
        <div class="input-group rounded my-3">
            <button style="background-color: #eb3748; border: none;">
                <i class="fas fa-search"></i>
            </button>
            <input type="search" width="200px" style="background-color: #eb3748; color: black; outline: none; box-shadow: none;" id="search" class="form-control rounded" placeholder="Search for course content" aria-label="Search" aria-describedby="search-addon" />

        </div>
        <a class="px-3 pt-2 mydivheader" href="{{ route('detailsStudents.show',Auth::guard('student')->user()->id) }}">
            <svg xmlns="http://www.w3.org/2000/svg" width='50' height='50' class="" viewBox="0 0 384 512"><!--!Font Awesome Free 6.5.1 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.-->
    ...
    <div class="wrapper" @if (!isset($GTest)) style="margin-top:70px;" @endif>
        @if (!isset($GTest))
        <aside style="height:100px;position: fixed;top:70px;background-color:white;" class="main-sidebar sidebar-dark-primary elevation-4  ">
            <!-- Sidebar -->
                ...
                <div class="accordion mt-2" id="accordionExample">
                    ...
                    <div id="itemAccordion" class="accordion-item">
                        <h2 class="accordion-header" id="heading{{ $curriculum->title }}">
                            <button id="accordion" class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapse{{ $curriculum->title }}" aria-expanded="true" aria-controls="collapse{{ $curriculum->title }}">
                                <b>{{ $curriculum->title }}</b>
                            </button>
                        </h2>
                        <div style="background-color:#07294D;color:white;" id="collapse{{ $curriculum->title }}" class="accordion-collapse collapse show" aria-labelledby="heading{{ $curriculum->title }}" class='data' data-bs-parent="#accordionExample">
                            <div class="accordion-body" id="accordion-body">
                                ...
                                <ol class="curriculum custom-ordered-list">
                                    ...
                                    <form id="lessonForm" action="{{ route('curricula.show', $curriculum->course_id) }}" method="post">
                                        @csrf
                                        ...
                                        <div class="list">
                                            <svg fill="#FFFFFF" width="20px" height="20px" viewBox="-3.5 0 19 19" xmlns="http://www.w3.org/2000/svg" class="cf-icon-svg mr-1">
                                                <path d="M11.16 16.153a.477.477 0 0 1-.476.475H1.316a.477.477 0 0 1-.475-.475V3.046a.477.477 0 0 1 .475-.475h6.95l2.893 2.893zm-1.11-9.924H8.059a.575.575 0 0 1-.574-.574V3.679H1.95v11.84h8.102zM3.907 4.92a1.03 1.03 0 1 0 1.029 1.03 1.03 1.03 0 0 0-1.03-1.03zm4.958 3.253h-5.87v1.108h5.87zm0 2.354h-5.87v1.109h5.87zm0 2.354h-5.87v1.109h5.87z" fill-rule="evenodd" />
                                            </svg>
                                            <li id="lessonItem" style="cursor: pointer;" class="lesson-item text-light d-inline lessonItem" style="background-color: #07294D;">
                                                {{ $lesson['title'] }} <br>
                                                @if ($lesson['view'] == 0)
                                                <span class="text-white-50" style="font-size:12px;margin-left:9rem; ">{{ $lesson['duration'] }}<i class="fa-light fa-eye ml-2" style="font-size: 18px; display: inline;"></i></span>@else<span class="text-white-50" style="font-size:12px;margin-left:9rem; ">
                                                    {{ $lesson['duration'] }}<i style="font-size: 18px; display: inline;" class="fa-solid fa-check ml-2"></i></span>
                                                @endif
                                            </li>
                                        </div>
                                    </form>
                                    ...
                                    <form id="lessonForm" action="{{ route('curricula.show', $curriculum->course_id) }}" method="post">
                                        ...
                                        <input type="hidden" value="{{ $lesson['type'] }}" name="type" />
                                        <div class="list">
                                            <svg fill="#FFFFFF" width="15px" height="15px" class="cf-icon-svg" viewBox="0 0 1920 1920" xmlns="http://www.w3.org/2000/svg">
                                            ...
                                            <li id="lessonItem" style="cursor: pointer;" class="lesson-item text-light d-inline lessonItem" style="background-color: #07294D;">
                                                {{ $lesson['title'] }} <br>
                                                @if ($lesson['pass'] === null)
                                                <span class="text-white-50" style="font-size:12px;margin-left:9rem; display: inline;">{{ $lesson['duration'] }}{{ substr($lesson['duration_unit'], 0, 1) }}
                                                    <i class="fa-regular fa-lock ml-2" style="font-size: 18px; display: inline;"></i></span>
                                                @elseif($lesson['pass'] == 1)
                                                <span class="text-white-50" style="font-size:12px;margin-left:9rem; ">
                                                    {{ $lesson['duration'] }}
                                                    {{ substr($lesson['duration_unit'], 0, 1) }}<i style="font-size: 18px; display: inline;" class="fa-solid fa-check ml-2"></i></span>
                                                @else
                                                <span class="text-white-50" style="font-size:12px;margin-left:9rem; ">
                                                    {{ $lesson['duration'] }}
                                                    {{ substr($lesson['duration_unit'], 0, 1) }}<i style="font-size: 18px; display: inline;" class="fa-solid fa-xmark ml-2"></i></span>
                                                @endif
                                            </li>
        ...
        <nav style="position:sticky;top: @if (!isset($GTest)) 70px; @else 0px; @endif" class="main-header navbar navbar-expand  navbar-light">
            <ul class="navbar-nav ">

                <li class="nav-item">
                    <a class="nav-link" @if (!isset($GTest)) data-widget="pushmenu" @endif href="#" role="button"><i class="fas fa-bars"></i></a>
                </li>
                    ...
                    <div class="container-fluid m--8">
                        <div class="px-8 py-4 d-flex justify-content-center ">
                            ...
                            <div class="d-flex flex-column" id="video" style="width: 810px;">
                                <iframe class="mt-3" width="100%" height="465" src="https://www.youtube.com/embed/{{ $lessonActive->video_link }}" title="YouTube video player" frameborder="0" allowfullscreen></iframe>
                                <h3 class='my-3 title text-uppercase'><b>{{ $lessonActive->title }}</b></h3>
                                <p class='mb-3 text-gray'>{{ $lessonActive->description }}</p>
                                <div style="background-color:#F4F6F9;" class="d-flex justify-content-center w-100 navbar p-2  ">
                                    <div class="jsutify-self-start" style="margin-bottom:0 !important;">
                                        <form class="d-inline" action="{{ route('curriculum_list.prev', $course->id) }}" method="post">
                                            @csrf
                                            <input type="hidden" value="{{ $lessonActive->order }}" name="order">
                                            <input type="hidden" value="{{Auth::guard('student')->user()->id}}" name="student_id">
                                            <input type="hidden" value="{{ $lessonActive->id }}" name="lesson_id">
                                            <button class="btn btn-primary rounded btnPag"><svg xmlns="http://www.w3.org/2000/svg" width="20px" height="20px" viewBox="0 0 256 512">
                                                    <path d="M9.4 278.6c-12.5-12.5-12.5-32.8 0-45.3l128-128c9.2-9.2 22.9-11.9 34.9-6.9s19.8 16.6 19.8 29.6l0 256c0 12.9-7.8 24.6-19.8 29.6s-25.7 2.2-34.9-6.9l-128-128z" fill="white" />
                                                </svg> Prev</button>
                                        </form>
                                    </div>
                                    <div class="justify-self-end ms-auto" style="margin-bottom:0 !important;">
                                        <form class="d-inline" action="{{ route('curriculum_list.next', $course->id) }}" method="post">
                                            @csrf
                                            <input type="hidden" value="{{ $lessonActive->order }}" name="order">
                                            <input type="hidden" value="{{Auth::guard('student')->user()->id}}" name="student_id">
                                            <input type="hidden" value="{{ $lessonActive->id }}" name="lesson_id">
                                            <button class="btn btn-primary rounded btnPag">
                                                Next <svg xmlns="http://www.w3.org/2000/svg" width="20px" height="20px" viewBox="0 0 256 512">
                                                    <path d="M246.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-128-128c-9.2-9.2-22.9-11.9-34.9-6.9s-19.8 16.6-19.8 29.6l0 256c0 12.9 7.8 24.6 19.8 29.6s25.7 2.2 34.9-6.9l128-128z" fill="white" />
                                                </svg>
                                            </button>
                                        </form>
                                    </div>
                            ...
                            <center id="content-quiz">
                                <div id="divquiz">
                                    ...
                                    @if ($retaking <= $quizActive->retake)
                                        <form id="formQuiz" action="{{ route('checkQuiz', $course->id) }}" method="post">
                                            @csrf
                                            @if (isset($GTest))
                                            ...
                                            <section style="width: 1000px;">
                                                ...
                                                <div class="question" data-bs-parent="{{ $question->id }}">
                                                    ...
                                                    <div class="option_list">
                                                        ...
                                                        @if (!isset($answers))
                                                        <div class="option data d-flex justify-content-start ml-3" data-bs-parent="{{ $question->question_type }}">
                                                            <input name="radio{{ $question->id }}" value="{{ $option->id }}" class=" mr-3" style="width: 20px;height:20px;" type="radio"><span>{{ $option->option_text }}</span>
                                                        </div>
                                                        @endif
                                                        @isset($answers)
                                                        @if ($option->is_correct == 1)
                                                        <div class="option selected data d-flex justify-content-start ml-3" data-bs-parent="{{ $question->question_type }}">
                                                            <input checked name="radio{{ $question->id }}" value="{{ $option->id }}" class=" mr-3" style="width: 20px;height:20px;" type="radio"><span>{{ $option->option_text }}</span>
                                                        </div>
                                                        @else
                                                        @foreach ($answers as $answer)
                                                        @if ($answer->option_id == $option->id && $answer->question_id == $option->question_id && $answer->is_correct == 0)
                                                        <div class="option false data d-flex justify-content-start ml-3" data-bs-parent="{{ $question->question_type }}">
                                                            <input name="radio{{ $question->id }}" value="{{ $option->id }}" class=" mr-3" style="width: 20px;height:20px;" type="radio"><span>{{ $option->option_text }}</span>
                                                            <i class="fa-solid fa-ban text-danger ms-auto fs-4"></i>
                                                        </div>
                                                        @endif
                                                ...
                                                <div class="question" data-bs-parent="{{ $question->id }}">
                                                    ...
                                                    <div class="option_list">
                                                        ...
                                                        @if (!isset($answers))
                                                        <div class="option data d-flex justify-content-start ml-3" data-bs-parent="{{ $question->question_type }}">
                                                            <input style="width: 20px;height:20px;" class=" mr-3" value="{{ $option->id }}" id="check{{ $key + 1 }}" type="checkbox"><span for="check{{ $key + 1 }}">{{ $option->option_text }}</span>
                                                        </div>
                                                        @endif
                                                        ...
                                                        @endphp
                                                        @if ($option->is_correct == 1)
                                                        <div class="option data selected d-flex justify-content-start ml-3" data-bs-parent="{{ $question->question_type }}">
                                                            <input checked style="width: 20px;height:20px;" class=" mr-3" value="{{ $option->id }}" id="check{{ $key + 1 }}" type="checkbox"><span for="check{{ $key + 1 }}">{{ $option->option_text }}</span>
                                                        </div>
                                                        @elseif($found)
                                                        {{-- @foreach ($answers as $answer)
                                                                @if ($answer->option_id == $option->id && $answer->question_id == $option->question_id && $answer->is_correct == 0) --}}
                                                        <div class="option data false d-flex justify-content-start ml-3" data-bs-parent="{{ $question->question_type }}">
                                                            <input checked style="width: 20px;height:20px;" class=" mr-3" value="{{ $option->id }}" id="check{{ $key + 1 }}" type="checkbox"><span for="check{{ $key + 1 }}">{{ $option->option_text }}</span>
                                                            <i class="fa-solid fa-ban text-danger ms-auto fs-4"></i>
                                                        </div>
                                                        @else
                                                ...
                                                <div class="gauge">
                                                    <div class="gauge__body">
                                                        <div class="gauge__fill" style="background: #E72929 !important"></div>
                                                        <div class="gauge__cover"></div>
                                                    </div>
                                                </div>
                                                ...
                                                @endif
                                                <div id="error" style="display: none;" class="btn__display top element-color-control-icon">
                                                    <div class="text-danger has-error" role="content" data-sider-select-id="a76800fe-ece4-418b-ae38-7e80bab89cb6">
                                                        <i class="fas fa-warning text-danger mr-2"></i>Please
                                                        answer
                                                        all
                                                        the above questions before clicking 'Submit'<div class="unanswered-qustions-number" data-sider-select-id="552ee91b-eb56-4474-a424-d7b8e311c914">
                                                            <div>Unanswered question number:</div><span id="bnquestion"></span>
                                                        </div>
                                                    </div>
                                                </div>
                                                <br>
                                                ...
                                                <button style="padding: 2% 20% 2% 20% ;" id="commit" type="button" class="btn btn-primary retake">Submit<span style="font-size: 12px;">
                                                        Retaking({{ $quizActive->retake - $retaking }})</span></button>
                                                @elseif (!isset($move))
                                                <button style="padding: 2% 20% 2% 20% ;" id="commit" type="button" class="btn btn-primary">Submit</button>
                                                @endif
                                            </section>
                                        </form>
                                        @if ($pass == 1 && $move)
                                        <form id='formFinishQ' style="display: inline !important;" class="d-inline formFinishQ" action="{{ route('curriculum_list.next', $course->id) }}" method="post">
                                            @csrf
                                            <input type="hidden" value="{{ $quizActive->order }}" name="order">
                                            <input type="hidden" value="{{Auth::guard('student')->user()->id}}" name="student_id">
                                            <input type="hidden" value="{{ $quizActive->id }}" name="lesson_id">
                                        ...
                                        <div class="alert alert-success" role="alert">
                                            ...
                                            <button type="button" class="btn btn-primary btn-sm" onclick="goBack()">Go
                                                Back</button>
                                        </div>
                                        @endif
                                        @endisset
                ...
                <div class="accordion-body">`;
                    ...
                    <form id="lessonForm" action="/curriculum_list/${cr.course_id}" method="post">
                        ...
                        <div class="list">
                            ...
                            ${lesson.title}<br>
                            ${lesson.view == 0 ?
                            `<span class="text-white-50" style="font-size:12px;margin-left:13rem ;">${lesson.duration}${lesson.duration_unit.charAt(0)} <i class="fa-light fa-eye ml-2" style="font-size: 18px; display: inline;"></i></span>` :
                            `<span class="text-white-50" style="font-size:12px;margin-left:13rem ;">${lesson.duration}${lesson.duration_unit.charAt(0)} <i style="font-size: 18px; display: inline;" class="fa-solid fa-check ml-2"></i></span>`
                        }
                        </li>
                        </div>
                    </form>`;
                            ...
                            if (quizzes.length !== 0) {
                                ...
                                <div class="list"><svg fill="#FFFFFF" width="15px" height="15px" class=' mr-1'
                                                            ...
                                                            ${lesson.title} <br>
                                                            ${lesson.pass === null ?
                    `<span class="text-white-50" style="font-size:12px;margin-left:13rem ; display: inline;">${lesson.duration}${lesson.duration_unit.charAt(0)} <i class="fa-regular fa-lock ml-2" style="font-size: 18px; display: inline;"></i></span>` :
                    (lesson.pass == 1 ?
                        `<span class="text-white-50" style="font-size:12px;margin-left:13rem ;">${lesson.duration}${lesson.duration_unit.charAt(0)} <i style="font-size: 18px; display: inline;" class="fa-solid fa-check ml-2"></i></span>` :
                        `<span class="text-white-50" style="font-size:12px;margin-left:13rem ;">${lesson.duration}${lesson.duration_unit.charAt(0)} <i style="font-size: 18px; display: inline;" class="fa-solid fa-xmark ml-2"></i></span>`
                    )
                }
                                                        </li>
                                                        </div>
